...
def plot_predictions_ML(start_date, end_date, num_days_predicted):
    ...
    )  # Set background color to match Streamlit background

    ax.plot(
        filtered_predictions["date"],
        filtered_predictions["actual_energy_price"],
    ...
    ax.plot(
        filtered_predictions_pred["date"],
        filtered_predictions_pred["predicted_energy_price"],
