...
# Define a command for plotting correlation matrix
@app.command()
def plot_correlation_matrix():
    ...
    features_cor = data[list(data.columns[1:35])]
    ...
    sns.heatmap(
        round(features_cor.corr(), 1), annot=True, cmap="Greens", linewidth=0.9, ax=ax
    )
    ax.tick_params(
        axis="x", colors="#1c0858", labelsize=30
    )  # X-axis ticks in white color
    ax.tick_params(
        axis="y", colors="#1c0858", labelsize=30
    )  # Y-axis ticks in white color
    st.pyplot(fig)
